...
internal class Program
{
    ...
    private static List<Villain> enemy;
    ...
    static void GameDataSetting()
    {
        ...
        //적 정보 세팅
        enemy = new List<Villain>
        
        {
            new Villain("미니언",EnemyType.Minian, 2, 15, 10, 15),
            new Villain("대포미니언",EnemyType.Mopo, 2, 15, 10, 45),
            new Villain("공허충 무리",EnemyType.Void, 3, 10, 20, 50)
        };
    ...
    static void EnterDungeon()
    {
        ...
        {
            Console.WriteLine($"=== 던전 {floor}층 ===");
            Console.WriteLine();

            // 랜덤으로 적 생성
            Villain enemy = GenerateRandomEnemy();

            Console.WriteLine($"{enemy.Name}이(가) 나타났습니다!");
            Console.WriteLine();
            ...
            // 던전 클리어 메시지
            Console.WriteLine($"던전 {floor}층을 클리어했습니다!");
            Console.WriteLine();
        }

        Console.WriteLine("모든 던전을 클리어했습니다! 축하합니다!");

        // 돌아가기
        Console.WriteLine("게임으로 돌아가려면 아무 키나 눌러주세요.");
        Console.ReadKey();
        DisplayGameIntro();
    }

    static Villain GenerateRandomEnemy()
    {
        Random random = new Random();

        // 적의 종류를 랜덤으로 선택
        EnemyType randomEnemyType = (EnemyType)random.Next(Enum.GetValues(typeof(EnemyType)).Length);

        // 랜덤한 레벨, 공격력, 방어력, 체력
        int randomLevel = random.Next(1, 4);
        int randomDamage = random.Next(10, 20) * randomLevel;
        int randomDef = random.Next(5, 15) * randomLevel;
        int randomHp = random.Next(30, 50) * randomLevel;

        // 랜덤한 적 생성
        Villain enemy = new Villain($"적 Lv.{randomLevel} ({randomEnemyType})", randomEnemyType, randomLevel, randomDamage, randomDef, randomHp);

        return enemy;
    }
    static void Battle(Villain enemy)
    {
        // 전투 로직 작성
...
public class Villain
{
    ...
    public Villain(string name, EnemyType enemyType, int level ,int damage, int def,int hp)
    {
        Name = name;
        Level = level;
        Damage = damage;
